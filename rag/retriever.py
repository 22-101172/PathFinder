...
class HybridRetriever:
      def __init__(self):
                ...
                self.child_db = Chroma(persist_directory=PERSIST_DIR, embedding_function=self.embeddings)
    ...
    def retrieve(self, query: str, k_vec: int = 20, k_bm25: int = 15, k_final: int = 6) -> list:
        ...
        top_idx = sorted(range(len(bm25_scores)), key=lambda i: bm25_scores[i], reverse=True)[:k_bm25]
