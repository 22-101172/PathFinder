...
from sae.sae_adapter import SAEAdapter
from sae.feature_engineering import compute_student_features, build_features
from sae.rule_engine import (
    f1_gpa_trend,
    f2_credit_pace,
    f3_risk_flags,
...
def _load_course_names() -> dict[str, str]:
    try:
        path = Path(__file__).parent.parent / "data" / "Course_Catalogue_Correct_Version.xlsx"
        if not path.exists():
            return {}
